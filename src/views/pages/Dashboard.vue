<template>
  <v-container id="dashboard" tag="section" fill-height fluid>
    <v-row class="my-10">
      ...
      <v-col cols="12" md="6" sm="6">
        <base-material-card color="grey darken-2" class="px-5 py-3">
...
export default {
  ...
  methods: {
    ...
    async getRemainingStockCost() {
      try {
        let { data, error } = await supabase.from("cost_rem_stock").select("*");
        if (error) {
          console.log(error);
        } else {
          console.log(data);
          this.total_remaining_stock = formatPHP.format(data[0].sum);
    ...
    async getUsedCost() {
      try {
        let { data, error } = await supabase.from("total_exp_cost").select("*");
        if (error) {
          console.log(error);
        } else {
          console.log(data);
          this.total_exp_cost = formatPHP.format(data[0].sum);
    ...
    async initializeHighUtil() {
      try {
        ...
        if (error) {
          ...
          this.highest_util.item_util = Math.round(
            highest_item_util[0].item_util
          ).toString();
        }
      } catch (error) {
        console.log(error);
    ...
    async downloadCSVInventoryOverview() {
      try {
        ...
        if (data) {
          ...
          anchor.download = "InventoryOverview.csv";
          anchor.click();
        } else {
