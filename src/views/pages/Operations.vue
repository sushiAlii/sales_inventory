      ...
      <v-col cols="12" md="2">
        ...
        >
          ...
          <v-date-picker v-model="dates" range width="300">
            ...
            <v-btn text color="primary" @click="loadNewOperations">
              OK
            </v-btn>
          </v-date-picker>
        </v-menu>
...
export default {
  ...
  methods: {
    async loadOperations() {
      this.loading = true;
      let { data, error } = await supabase.from("operation_view").select("*");
    ...
    async loadNewOperations() {
      ...
      this.menu = false;
      let { data, error } = await supabase.rpc("getfilterdoperation", {
